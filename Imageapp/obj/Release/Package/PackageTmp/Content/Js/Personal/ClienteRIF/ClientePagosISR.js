const mdlPagosISR = function () {
    //! Doom del modal
    var $formPagos = $("#frmPagosISR"),
        $modalPagos = $("#mdlPagosISR"),        
        eventModal = 'BTN-ADD-PAGOS-ISR',
        tblPrincipal = null,
        objOriginal = {};

    //! Campos modal [CRUD]
    //var $fr_id_cliente = $("#fr_id_paciente");
    var $fr_fecha_pago_sat_isr = $("#fr_fecha_pago_sat_isr"); //la descripcion del modal 
    var $fr_bimestre_pago_sat_isr = $("#fr_bimestre_pago_sat_isr");
    var $fr_periodo_bimestre_pago_sat_isr = $("#fr_periodo_bimestre_pago_sat_isr");
    var $fr_monto_pago_sat_isr = $("#fr_monto_pago_sat_isr");
    ...
    function inicio() {
        tblPrincipal = objClsClienteRIF.tblPagosISR;
    }
    ...
    async function setDatosModal() {
        ...
        objClsClienteRIF.tblPagosISR.adjustColumns();//renderiza mejor las columnas
    ...
    function GetPagosCliente(showWait = false) {
        ...
        let id_RVA = objClsClienteRIF.getlistaPermisos()["BTN-ADD-PAGOS-ISR"].id_RVA;


        //Peticion de la lista de registros para Repositorio Pacientes
        doAjax("POST", url_Catalogos_GetPagosISR, { jsonJS: objSend, id_RV: id_RV, id_RVA: id_RVA }, showWait).done(function (data) {
            ...
            if (result.validResult() && result.resultStoredProcedure.validResultStored()) {
                ...
                contenido.forEach(function (item) {
                    ...
                    item.acciones = '<a href="#" onclick="objClsClienteRIF.mdlPagosISRCliente.recalcular(' + item.id_bimestre + ',' + item.periodo + ',' + item.id_pago + ',true)" class="btn btn-style btn-success" title="recalcular"><i class="fa fa-refresh"></i ></a>&nbsp;&nbsp;';
                    //item.acciones += '<a href="#" class="btn btn-style btn-warning" title="editar pago"><i class="fa fa-edit"></i ></a>&nbsp;&nbsp;';
                    item.acciones += '<a href="#" onclick="objClsClienteRIF.mdlPagosISRCliente.eliminar(' + item.id_pago + ',true)" class="btn btn-style btn-danger" title="eliminar pago"><i class="fa fa-trash"></i ></a>';
                });
    ...
    function recalcular(bimestre, periodo, id_pago, showWait = false) {
        ...
        doAjax("POST", url_Catalogos_RecalcularISR, { jsonJS: objSend, id_RV: id_RV, id_RVA: id_RVA }, showWait).done(function (data) {
            let result = new Result(data);
    ...
    function eliminar(id_pago, showWait = false) {
        ...
        doAjax("POST", url_Catalogos_EliminarPagoISR, { jsonJS: objSend, id_RV: id_RV, id_RVA: id_RVA }, showWait).done(function (data) {
            let result = new Result(data);
    ...
    function getDatosFomulario() {
        ...
        objSendStored.fecha_pago_sat = $fr_fecha_pago_sat_isr.val();
        objSendStored.bimestre_pago_sat = $fr_bimestre_pago_sat_isr.val();
        objSendStored.monto_pago_sat = $fr_monto_pago_sat_isr.val();
        objSendStored.periodo_bimestre_pago_sat = $fr_periodo_bimestre_pago_sat_isr.val();
    ...
    function enviarStoredProcedure() {
        let objSend = {};
        //! Valildar formulario
        if (eventModal == 'BTN-ADD-PAGOS-ISR') {
            if (!$formPagos.valid())
    ...
    function inicializarValidaciones() {
        ...
        principalForm = {
            rules: {
                fr_fecha_pago_sat_isr: "required",
                fr_bimestre_pago_sat_isr: "required",
                fr_monto_pago_sat_isr: "required",
                fr_periodo_bimestre_pago_sat_isr: "required"
            },
            messages: {
                fr_fecha_pago_sat_isr: "Ingrese una fecha",
                fr_bimestre_pago_sat_isr: "Ingrese un bimestre",
                fr_monto_pago_sat_isr: "Ingrese un monto",
                fr_periodo_bimestre_pago_sat_isr: "Ingrese un año"
            }
        }
