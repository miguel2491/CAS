const mdlEstatusCliente = function () {
    //! Doom del modal
    var $formEstatus = $("#frmEstatus"),
        $modalEstatus = $("#mdlEstatus"),
        eventModal = 'BTN-ESTATUS',
        tblPrincipal = null,
        objOriginal = {};

    //! Campos modal [CRUD]
    //var $fr_id_cliente = $("#fr_id_paciente");
    var $fr_rfc_estatus = $("#fr_rfc_estatus"); //la descripcion del modal 
    var $fr_estatus_cliente = $("#fr_estatus_cliente");
    var $fr_observacion_cliente = $("#fr_observacion_cliente");
    var $fr_fecha_estatus_cliente = $("#fr_fecha_estatus_cliente");
    ...
    function inicio() {
        tblPrincipal = objClsCliente.tblEstatus;
    }
    ...
    async function setDatosModal() {
        ...
        $modalEstatus.modal();//Mostrar modal
        await sleep(200);//Dale tiempo para mostrar el modal
        objClsCliente.tblEstatus.adjustColumns();//renderiza mejor las columnas
        GetEstatusCliente(true);//obtiene la lista de repositorios del paciente y los muestra en el modal
    }
    ...
    function GetEstatusCliente(showWait = false) {
        tblPrincipal.clearRows();
        let objSend = JSON.stringify({ id_cliente: id_cliente });
        let id_RVA = objClsCliente.getlistaPermisos()["BTN-ESTATUS"].id_RVA;


        //Peticion de la lista de registros para Repositorio Pacientes
        doAjax("POST", url_Catalogos_GetEstatus, { jsonJS: objSend, id_RV: id_RV, id_RVA: id_RVA }, showWait).done(function (data) {
            ...
            if (result.validResult() && result.resultStoredProcedure.validResultStored()) {
                ...
                contenido.forEach(function (item) {
                    ...
                    item.acciones = '<a href="#" onclick="objClsCliente.mdlEstatus.eliminar(' + item.id_estatus_cliente + ')" class="btn btn-style btn-danger" title="eliminar actividad"><i class="fa fa-trash"></i ></a>';
                });
    ...
    function eliminar(id_estatus_cliente, showWait = false) {
            ...
            buttons: {
                SI: function () {
                    let id_RVA = objClsCliente.getlistaPermisos()["BTN-ACTIVIDAD"].id_RVA;
                            var objSend = JSON.stringify({ id_estatus_cliente: id_estatus_cliente });
                            doAjax("POST", url_Catalogos_EliminarEstatus, { jsonJS: objSend, id_RV: id_RV, id_RVA: id_RVA }, showWait).done(function (data) {
                                let result = new Result(data);
    ...
    function setDatosFormulario() {
        ...
        ////! Asignar datos
        $fr_rfc_estatus.val(tableRow.rfc);
    ...
    function getDatosFomulario() {
        ...
        objSendStored.estatus = $fr_estatus_cliente.val();
        objSendStored.observacion = $fr_observacion_cliente.val();
        objSendStored.fecha = $fr_fecha_estatus_cliente.val();        
    ...
    function enviarStoredProcedure() {
        let objSend = {};
        //! Valildar formulario
        if (eventModal == 'BTN-ESTATUS') {
            if (!$formEstatus.valid())
    ...
    async function finalizarStoredProcedure(data) {
        ...
        if (result.validResult() && result.resultStoredProcedure.validResultStored(false, true)) {
            ...
            jsSimpleAlert("Correcto", result.resultStoredProcedure.msnSuccess, "green");
            GetEstatusCliente(true);
            objClsCliente.getListaPrincipal(false);
            //clearData();
            //! INTEGRAR REGISTRO AL SERVIDOR PRINCIPAL
            //IntegrarAlServidor(url_Integracion_SetIntegracion_Servidor, result.resultStoredProcedure.newGuid);
        }
    ...
    function resetearModal() {
        $formEstatus.validate().resetForm();
        $formEstatus[0].reset();
        tblPrincipal.clearRows();
        setDatosFormulario();
    }
    ...
    function inicializarValidaciones() {
        ...
        principalForm = {
            rules: {
                fr_fecha_estatus_cliente: "required"

            },
            messages: {
                fr_fecha_estatus_cliente: "Ingrese una fecha"

            }
        }
        $formEstatus.validate(principalForm);
    }
