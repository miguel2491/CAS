const mdlFechaPago = function () {
    ...
    var $fl_fecha_pago = $("#fl_fecha_pago");
        ...
        arrayData.forEach(function (item) {
            if (item.isSelected == 1) { //! Solo los elementos seleccionados            
                arrayLista.push(new Promise((resolve, reject) => {
                    doAjax("POST", url_Catalogo_SetFechaPago, { jsonJS: JSON.stringify({ id_xml: item.id_xml, fecha_pago: $fl_fecha_pago.val() }), id_RV: id_RV }).done(function (data) {
                        conta++;
