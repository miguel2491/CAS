...
namespace sat_ws
{
    class ReadXMLCFDI32Nomina11ToObject
    {
        ...
        public static bool Get( DataSet dsXMLCFDI, DataSet dsXMLCFDINomina,  XDocument xmlDoc, string version, string RutaXML)
        {
            ...
            try
            {
                ...
                foreach (DataRow dato in dsXMLCFDI.Tables[8].Rows)
                {
                ...
                string ConceptoDeduccion = "";
                //Inicializando HTDeducciones
                for (int m = 1; m < 22; m++)
                {
                    HTDeducciones.Add(m + "-C", "0.00");
                    HTDeducciones.Add(m + "-G", "0.00");
                    HTDeducciones.Add(m + "-E", "0.00");
                    HTDeducciones.Add(m + "-T", "0.00");
                    foreach (DataRow dato in dsXMLCFDINomina.Tables["Deduccion"].Rows)
                    {
                        if (Convert.ToInt16(dato["TipoDeduccion"].ToString()) == m)
                        {
                            ...
                            ConceptoDeduccion = dato["Concepto"].ToString();
                        }                        
                    }
                    HTDeducciones[m.ToString() + "-C"] = ConceptoDeduccion;
                    HTDeducciones[m.ToString() + "-G"] = SumDeduccionGravada;
                    HTDeducciones[m.ToString() + "-E"] = SumDeduccionExcenta;
                    HTDeducciones[m.ToString() + "-T"] = SumDeduccionExcenta + SumDeduccionGravada;
                ...
                GeneraPDFNomina.GeneraRepresentacionImpresa(RutaXML.Replace(".xml", ".pdf").Replace(".XML", ".pdf"), CFDINomina, HTPercepciones, HTDeducciones, version);
